...
PostgreSQL             price_daily / financials / derived_metrics /
   │                   canslim_scores / security_status_daily 등
   ▼
...
### DB 확인
```bash
psql -U canslim_user -d canslim -c "SELECT status_date, count(*) FROM security_status_daily GROUP BY status_date ORDER BY status_date DESC LIMIT 3;"
```
