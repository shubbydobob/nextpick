...
public interface NextpickScoreRepository extends JpaRepository<NextpickScore, Long> {

    Optional<NextpickScore> findBySecurityIdAndScoreDate(Long securityId, LocalDate scoreDate);

    List<NextpickScore> findByScoreDateAndMarketOrderByCompositeScoreDesc(
            LocalDate scoreDate, String market);

    Page<NextpickScore> findByScoreDateAndMarket(
            LocalDate scoreDate, String market, Pageable pageable);

    long countByScoreDateAndMarket(LocalDate scoreDate, String market);

    List<NextpickScore> findBySecurityIdOrderByScoreDateDesc(Long securityId);

    Optional<NextpickScore> findFirstBySecurityIdOrderByScoreDateDesc(Long securityId);

    /** 시장의 가장 최근 채점일 조회 */
    Optional<NextpickScore> findFirstByMarketOrderByScoreDateDesc(String market);

    @Modifying
    @Query(value = """
        INSERT INTO nextpick_scores
    ...
    @Query(value = """
        ...
        WHERE nextpick_scores.id = ranked.id
        """, nativeQuery = true)
